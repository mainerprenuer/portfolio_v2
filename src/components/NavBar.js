...
import { TwitterIcon, LinkedInIcon, GithubIcon, SunIcon, MoonIcon } from './Icons'
...
const CustomLink = ({ href, title, className='' }) => {
  ...
  return(
    <Link href={href} className={`${className} relative group`}>
      ...
      <span className={`h-[1.5px] inline-block bg-dark absolute 
...
//hamburger hooks
  ...
  return (
   ...
   {/* hamburger menu */}
    <button className='flex-col juatify-center items-center hidden lg:flex' onClick={handleClick}>
       ...
       <span className={`bg-dark dark:bg-light block transition-all duration-300 ease-out h-0.5 w-6 rounded-sm translate-y-0.5 ${isOpen ? '-rotate-45 -translate-y-1' : 'translate-y-0.5'}`}></span>
        ...
        >
        {
          mode === "dark" 
          ? 
          <SunIcon className={"fill-dark"} onClick={handleClick}/> 
          :
          <MoonIcon className={"fill-dark"} onClick={handleClick}/>
        }
        </button>
